...
class SMPL:
    ...
    def animate(self, betas, pose, trans):
        ...
        def with_zeros(xs):
            ...
            """
            return tf.concat(
                [xs, tf.constant(np.tile(np.array([0, 0, 0, 1]).reshape([1, 1, 4]), [total, 1, 1]), dtype=tf.float32)],
                axis=1)
        ...
        for idx, pid in enumerate(self.parent_ids[1:]):
            A_global[idx + 1] = tf.matmul(A_global[pid],
                                          with_zeros(tf.concat([pose_mat[:, idx + 1, :3, :3],
                                                                tf.expand_dims(J[:, idx + 1] - J[:, pid], axis=2)],
                                                               axis=2)))
        ...
        A_tmp = [A_global[i] - pack(tf.matmul(A_global[i], tf.expand_dims(
            tf.concat([J[:, i], tf.constant(np.zeros([total, 1]), dtype=tf.float32)], axis=1), axis=-1))) for i in
                 range(len(A_global))]
        ...
        rest_shape_h = tf.concat([tf.transpose(v_posed), tf.constant(np.ones([1, 6890, total]), dtype=tf.float32)],
                                 axis=0)
...
def Rodrigues(rot_vs):
    ...
    tmp_0 = tf.stack([tf.multiply(x, x), tf.multiply(x, y), tf.multiply(x, z), tf.multiply(x, y), tf.multiply(y, y),
                      tf.multiply(y, z), tf.multiply(x, z), tf.multiply(y, z), tf.multiply(z, z)], axis=-1)
    tmp_0 = tf.reshape(tmp_0, [total, 3, 3])

    tmp_1 = tf.stack(
        [np.zeros([total, 1]), -1 * z, y, z, np.zeros([total, 1]), -1 * x, -1 * y, x, np.zeros([total, 1])], axis=-1)
    tmp_1 = tf.reshape(tmp_1, [total, 3, 3])

    rot_mats = tf.multiply(tf.reshape(c, [total, 1, 1]),
...
if __name__ == '__main__':
    ...
    optimizer = scipy_pt(loss=loss, var_list=[param_rot, param_shape, param_pose, param_trans],
